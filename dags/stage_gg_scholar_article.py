...
def stage_gg_scholar_article():
    ...
    s3_file_path = 'gg_scholar_article'
    ...
    conn.cursor().execute('CREATE OR REPLACE TABLE PC_DBT_DB.GG_SCHOLAR.GG_SCHOLAR_ARTICLE ( TITLE VARCHAR(300), LINK VARCHAR(200), CITATION_ID VARCHAR(25), AUTHORS VARCHAR(200), PUBLICATION VARCHAR(100), YEAR NUMBER(4,0), CITED_BY_VALUE NUMBER(10,0))')
    #conn.cursor().execute(f"COPY INTO PC_DBT_DB.GG_SCHOLAR.GG_SCHOLAR_LIST FROM @{stage_name}")
    conn.cursor().execute(f"COPY INTO PC_DBT_DB.GG_SCHOLAR.GG_SCHOLAR_ARTICLE FROM @{stage_name}")
