...
# Function to upload file to stage
def stage_gg_scholar_citations():
    # Load the .env file
    load_dotenv()
    ...
    s3_file_path = 'gg_scholar_citations_by_year.csv'
    ...
    conn.cursor().execute('CREATE OR REPLACE TABLE PC_DBT_DB.GG_SCHOLAR.GG_SCHOLAR_CITATION (AUTHOR_ID VARCHAR(12), CITED_YEAR NUMBER(4,0), CITATIONS NUMBER(38,0))')
    conn.cursor().execute(f"COPY INTO PC_DBT_DB.GG_SCHOLAR.GG_SCHOLAR_CITATION FROM @{stage_name}")
    
    print("stage gg_scholar_citations successfully loaded")
